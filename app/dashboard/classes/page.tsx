...
export default function ClassesPage() {
  ...
  async function loadData() {
    setLoading(true)
    const res = await fetch(`/api/classes?start=${period.start}&end=${period.end}&location=48718`)
  ...
  return (
    <div>
      <SecLabel>Hold — København</SecLabel>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <input type="date" value={period.end} onChange={e => setPeriod(p => ({ ...p, end: e.target.value }))}
          style={{ padding: '6px 12px', border: '1px solid #e4e0f0', borderRadius: 8, fontSize: 12, fontFamily: 'Inter, sans-serif', color: '#1a1520' }} />
        <input
          type="text" placeholder="Søg holdtype eller instruktør..."
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 24 }}>
        {[
          { label: 'Hold i alt', val: filtered.length },
      ...
      {loading ? (
        <div style={{ padding: 40, color: '#8a85a0', textAlign: 'center' }}>Henter hold fra Mariana Tek...</div>
      ) : filtered.length === 0 ? (
            ...
            .map(([date, daySessions]) => (
              <div key={date} style={{ marginBottom: 20 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: '#6b5ca5', letterSpacing: '.1em', textTransform: 'uppercase', marginBottom: 10, paddingBottom: 8, borderBottom: '2px solid #e4e0f0' }}>
                  ...
                  <span style={{ color: '#8a85a0', fontWeight: 500, marginLeft: 8 }}>· {daySessions.length} hold</span>
                </div>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                    ...
                    <tbody>
                      {daySessions
                        ...
                        .map((s, idx) => {
                          const pct = Math.round(s.participants / s.capacity * 100)
                          ...
                          return (
                            <tr key={idx} style={{ borderBottom: '1px solid #f0eef8' }}>
                              ...
                              <td style={{ padding: '10px 14px', fontFamily: 'Barlow Condensed, sans-serif', fontSize: 16, fontWeight: 700, color: '#1a1520' }}>{s.participants}</td>
                              ...
                              <td style={{ padding: '10px 14px' }}>
                                {pct >= 90 && <span style={{ fontSize: 9, padding: '2px 8px', borderRadius: 10, background: '#e8f5ef', color: '#2e8b6a', border: '1px solid #b0d8c4', fontWeight: 600 }}>Fuldt</span>}
                                {pct === 0 && <span style={{ fontSize: 9, padding: '2px 8px', borderRadius: 10, background: '#fdecea', color: '#c0392b', border: '1px solid #f0b8b0', fontWeight: 600 }}>Tomt</span>}
                              </td>
                            </tr>
                          )
                        })}
