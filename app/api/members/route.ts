...
export async function GET(request: Request) {
  ...
  // Gruppér men summer renewal_rate direkte per medlem — ikke count × én pris
  const grouped = paying.reduce((acc: any, t: any) => {
    ...
    acc[name].count++
    acc[name].mrr += t.renewal_rate  // summer direkte
    return acc
  }, {})

  const memberships = Object.entries(grouped).map(([name, d]: [string, any]) => ({
    name, count: d.count,
    price: Math.round(d.mrr / d.count), // gennemsnitspris til visning
    mrr: Math.round(d.mrr),             // præcis sum
  ...
  const totalMRR = paying.reduce((s, m) => s + (m.renewal_rate || 0), 0) // direkte sum
